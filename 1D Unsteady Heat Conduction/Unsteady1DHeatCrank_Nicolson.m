...
% rho*cp*DT/Dt=d/dx(kdT/dx)+S, From Patankar Book, Section 4-2-By Crank-Nicolson Method
...
Dt=20; % Time Step
NumberIt=1;  % Number of time steps
...
intAnimT=20; %Temperature Results are animated every intanimT Time Steps
intAnimQ=20; %Heat Flux Results are animated every intanimT Time Steps--May Be Slow Due to Calcualtions
...
while (IT<=NumberIt) && (err>epsT)
    ...
    for i=2:m-1
        ...
        ap0=(rho*Cp*Dx(i))/Dt;
        %ap
        ap=0.5*(ae+aw)+ap0-Sp*Dx(i);
        %Source Crank-Nicelson
        cn=(ap0-0.5*(ae+aw))*Told(i)+0.5*(aw*T(i-1)+ae*T(i+1));
        %b
        b(i)=Sc*Dx(i)+cn;
%         %Just for Checking Possibility of the Unphysical Behaviour of Crank Nicolson scheme
%         if (ap0-0.5*(ae+aw))<0
%             fprintf('\nCoefficient of Tp0(Should be Positive)= %2.3f \tcn=%2.6f\t\n',(ap0-0.5*(ae+aw)),cn)
%             pause
%         end
        %Assemble Soloution Matrix
        A(i,i)=ap;
