...
for j=1:M;
    ...
    switch NL %Dirichlet BC
        ...
        case 1    %Neuman BC on the Left
            Q(IJ)=Q(IJ)+BCl(j)*DY(j+1);%Ql  BCs at first CV
            AP(IJ)=AP(IJ)-AW(IJ);
            AW(IJ)=0;
    end
    i=N; %right Boundary
    ...
    switch NR
        case 0
            Q(IJ)=Q(IJ)+AE(IJ)*BCr(j)*DY(j+1);  %FI(NI,j)BCs at first CV
            AE(IJ)=0;
...
for i=1:N;
    ...
    switch NB  %Dirichlet BC
        ...
        case 1 %Neuman BC on the Bottom
            Q(IJ)=Q(IJ)+BCb(i)*DX(i+1);%   Qb  BCs at first CV
            AP(IJ)=AP(IJ)-AS(IJ);
            AS(IJ)=0;
    end
    j=M; %Top Boundary
    ...
    switch NT
        ...
        case 1 %Neuman BC on the Top
            Q(IJ)=Q(IJ)+BCt(i)*DX(i+1);   %Qt  %BCs at first CV
            AP(IJ)=AP(IJ)-AN(IJ);
            AN(IJ)=0;
    end
end
