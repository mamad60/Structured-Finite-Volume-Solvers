% This Scripts Solves 2D UnSteady Heat Transfer Equation-Explicit Time Stepping
...
Dt=1; % Time Step
NumberIt=100;  % Number of time steps
...
intAnimT=10; %Temperature Contour Results are animated every intanimT Time Steps
intAnimQ=10; %Heat Flux Results are animated every intanimT Time Steps--May Be Slow Due to Calcualtions
intAnimST=10; %Temperature Surface are animated every intanimT Time Steps
AVIOutputT=1; %1: Animation of  T Contour is Saved in AVI Format
AVIOutputQ=1; %1: Animation of  Q Contour is Saved in AVI Format
AVIOutputST=1; %1: Animation of  T Surface is Saved in AVI Format
...
MaxDt=(DX*DY)/(4*alpha); %Maximum Time Step for Stability
if Dt>MaxDt
...
while (IT<=NumberIt) && (err>epsT)
    ...
    %Apply BCs
    BCExplicit   %Customize this section for changing Problem
    %Interior Cells
    for i=2:m-1
        for j=2:n-1
            %Explicit Method
            switch tMethod
                case 0 %Explicit Euler
                    rhs=RHS(Told,i,j); %RHS of EQ.
                    T(i,j)=Euler(Told(i,j), Dt, rhs );
                case 1  %Explicit 2nd Order Rung-Kutta
                    T(i,j)=RK2(Told,Dt,i,j);
                case 2 %Explicit 4th Order Rung-Kutta
                    T(i,j)=RK4(Told,Dt,i,j);
            end
        end
    end
