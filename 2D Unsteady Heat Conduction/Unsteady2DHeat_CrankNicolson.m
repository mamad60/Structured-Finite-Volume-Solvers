% This Scripts Solves 2D UnSteady Heat Transfer Equation-Crank Nicolson Time Stepping
...
%------------
kStat=2;  % Themal Conductivity Calculation Method
...
while (IT<=NumberIt) && (err>epsT)
    ...
    for i=2:m-1
        for j=2:n-1
            ...
            %ap
            ap=ap0+0.5*(an+as+ae+aw)-Sp(i,j)*Dx(i,j)*Dy(i,j);
            %Source Crank-Nicelson
            cn=(ap0-0.5*(ae+aw+an+as))*Told(i,j)+0.5*(aw*T(i-1,j)+ae*T(i+1,j)+an*T(i,j-1)+as*T(i,j+1));
            % Set Labaling Coe.
            np=(j-1)*m+i;
            %b
            b(np)=Sc(i,j)*Dx(i,j)*Dy(i,j)+cn;
