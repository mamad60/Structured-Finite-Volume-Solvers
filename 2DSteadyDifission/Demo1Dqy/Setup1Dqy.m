function Setup1Dqy
% demo1Dqy  Verify 2D heat conduction code with a 1D problem.  Boundary conditions
%           are adjusted so that heat flow is in y-direction only.  Uniform q BC
%           is imposed on North boundary.  Constant T on South boundary.  Symmetry
%           on East and West.
...
FIn=80;   % FI at The Left Boundary, Applies only if Bn=0
FIs=30;   % FI at The Right Boundary, Applies only if Bs=0
%--------Neumann Bcs
qw=0; % Flux at The Left Boundary, Applies only if Bw=1
qe=0; % Flux at The Right Boundary, Applies only if Be=1
qn=(FIn-FIs)/(Ymax-Ymin);% Flux at The Left Boundary, Applies only if Bn=1
