...
% Half of the North Boundary is insulated and other is kept at constant temperature.
% The same BC is is applied to the north boundary. Other boundries are
% insulatied
% Note that for imposind these BCs AdjustBC & AdjustPost should be modified too.
...
Be=0; % 0:Fixed FI 1:Flux Bc--->East Boundary-->Actually mixed(Moidify AdjustBC)
Bn=0; % 0:Fixed FI 1:Flux Bc --->North Boundary->Actually mixed(Moidify AdjustBC)
Bs=1; % 0:Fixed FI 1:Flux Bc--->South Boundary
%----------Dirichlet BCs
T1 = 5; %East Boundary
T2 = 35; %North Boundary
FIw=0;   % FI at The Left Boundary, Applies only if Bw=0
FIe=T1;   % FI at The Right Boundary, Applies only if Be=0
FIn=T2;   % FI at The Left Boundary, Applies only if Bn=0
