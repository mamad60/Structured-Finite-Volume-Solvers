...
% demo2D  2D heat conduction in a rectangle.Constant q is imposed on the
% North Boundary. Remaining boundaris have constant T.
...
%----------Dirichlet BCs
FIw=pi;   % FI at The Left Boundary, Applies only if Bw=0
FIe=pi;   % FI at The Right Boundary, Applies only if Be=0
FIn=0;   % FI at The Left Boundary, Applies only if Bn=0
FIs=pi;   % FI at The Right Boundary, Applies only if Bs=0
%--------Neumann Bcs
qw=0; % Flux at The Left Boundary, Applies only if Bw=1
qe=0; % Flux at The Right Boundary, Applies only if Be=1
qn=sqrt(5); % Flux at The Left Boundary, Applies only if Bn=1
